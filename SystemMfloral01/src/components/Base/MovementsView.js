...
const MovementsView = ({ plants, hideForm, showOnlyForm }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.type === 'venta' || form.type === 'compra') {
      ...
      // --- NUEVO: Si es una venta, también guardar en la colección 'sales' ---
      if (form.type === 'venta') {
        const saleData = {
        ...
        try {
          ...
          console.error('Error guardando venta en sales:', err);
        }
      }
    } else {
  ...
  if (hideForm) {
    ...
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2">Movimientos de Caja - {now.toLocaleString('es-AR', { month: 'long', year: 'numeric' })}</h2>
